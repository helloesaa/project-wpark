import React from 'react';
import { Link } from 'react-router-dom';
import { useNavbar } from '../hooks/use-navbar';
import AuthModal from './AuthModal';

const Navbar = () => {
  const { 
    navbarData,
    user,
    location,
    open, setOpen, toggle,
    scrolled,
    showAuthModal, openLoginModal, closeLoginModal,
    dropdownOpen, setDropdownOpen, dropdownRef, handleLogout
  } = useNavbar();

  const { logo, menus } = navbarData;
  return (
    <>
      <header
        className={`fixed w-full top-0 z-[999] transition-all duration-300 ease-in-out ${
          scrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'
        }`}
      >
        <div className="px-6 lg:px-24 flex items-center justify-between">
          <Link to="/" className="flex items-center group relative z-[1001]">
            <div className="h-10 w-auto flex items-center transition-transform duration-300 group-hover:scale-105">
              <img src={logo} alt="Logo" className="h-10 object-contain rounded-md" />
            </div>
          </Link>

          <button
            onClick={toggle}
            className={`text-3xl lg:hidden transition relative z-[1001] ${
              scrolled || open ? 'text-gray-800' : 'text-white'
            }`}
          >
            <i className={`bx ${open ? 'bx-x' : 'bx-menu'} transform transition-transform duration-300 ${open ? 'rotate-90' : 'rotate-0'}`}></i>
          </button>

          <nav
            className={`
              absolute lg:static left-0 top-0 w-full lg:w-auto 
              transition-all duration-500 ease-in-out
              bg-white lg:bg-transparent
              ${open 
                ? 'max-h-screen opacity-100 pt-24 pb-8 shadow-2xl' /* Hapus overflow-hidden agar dropdown terlihat jika ada */
                : 'max-h-0 opacity-0 lg:max-h-full lg:opacity-100 lg:shadow-none lg:overflow-visible overflow-hidden'
              }
            `}
          >
            <ul className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-8 px-6 lg:px-0">
              
              {menus.map((item) => {
                const active = location.pathname === item.path;
                return (
                  <li key={item.label} className="w-full lg:w-auto relative group">
                    <Link
                      to={item.path}
                      onClick={() => setOpen(false)}
                      className={`
                        flex items-center gap-4 py-3 px-4 rounded-xl text-base font-medium transition-all duration-300
                        
                        /* Mobile Styles: Text Abu-abu, Hover Main */
                        text-gray-600 hover:bg-gray-50 hover:text-main hover:pl-6
                        ${active ? 'bg-main/10 text-main' : ''}

                        /* Desktop Styles */
                        lg:bg-transparent lg:hover:bg-transparent lg:hover:pl-4
                        ${scrolled 
                          ? (active ? 'lg:text-main lg:font-bold' : 'lg:text-gray-700 lg:hover:text-main') 
                          : (active ? 'lg:text-white lg:font-bold' : 'lg:text-white lg:hover:text-white/80')
                        }
                      `}
                    >
                      <i className={`bx ${item.icon} text-xl lg:hidden ${active ? 'text-main' : 'text-gray-400 group-hover:text-main'}`}></i>
                      {item.label}
                    </Link>

                    <span className={`hidden lg:block absolute bottom-0 left-4 h-0.5 transition-all duration-300 rounded-full
                      ${active ? 'w-[calc(100%-2rem)]' : 'w-0 group-hover:w-[calc(100%-2rem)]'}
                      ${scrolled ? 'bg-main' : 'bg-white'} 
                    `}></span>
                  </li>
                );
              })}

              <li className="mt-4 lg:mt-0 lg:ml-4 border-t border-gray-100 pt-4 lg:border-none lg:pt-0">
                
                {user ? (
                  <>
                    <div className="hidden lg:block relative" ref={dropdownRef}>
                      <button 
                        onClick={() => setDropdownOpen(!dropdownOpen)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-full border transition-all duration-300 ${
                          scrolled 
                            ? 'border-main text-main hover:bg-main hover:text-white' 
                            : 'border-white/50 text-white hover:bg-white hover:text-main'
                        }`}
                      >
                        <i className='bx bx-user-circle text-xl'></i>
                        <span className="font-medium max-w-[100px] truncate">{user.name}</span>
                        <i className={`bx bx-chevron-down transition-transform duration-300 ${dropdownOpen ? 'rotate-180' : ''}`}></i>
                      </button>

                      <div className={`
                        absolute right-0 mt-3 w-56 bg-white rounded-xl shadow-2xl py-2 transition-all duration-200 origin-top-right border border-gray-100
                        ${dropdownOpen ? 'opacity-100 scale-100 visible translate-y-0' : 'opacity-0 scale-95 invisible -translate-y-2'}
                      `}>
                        <div className="px-4 py-3 border-b border-gray-100 mb-1">
                          <p className="text-xs text-gray-400 mb-1">Login sebagai</p>
                          <p className="font-bold text-gray-800 truncate">{user.name}</p>
                          <p className="text-xs text-gray-500 truncate">{user.email}</p>
                        </div>
                        
                        <button 
                          onClick={handleLogout}
                          className="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-500 hover:bg-red-50 hover:text-red-600 text-left transition-colors rounded-b-xl"
                        >
                          <i className='bx bx-log-out'></i> Keluar
                        </button>
                      </div>
                    </div>
                    <div className="lg:hidden">
                      <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-xl mb-3 border border-gray-100">
                        <div className="w-10 h-10 rounded-full bg-main/10 flex items-center justify-center text-main">
                          <i className='bx bx-user text-2xl'></i>
                        </div>
                        <div className="overflow-hidden">
                          <p className="font-bold text-gray-800 truncate">{user.name}</p>
                          <p className="text-xs text-gray-500 truncate">{user.email}</p>
                        </div>
                      </div>
                      <button 
                        onClick={handleLogout}
                        className="w-full flex items-center justify-center gap-2 px-4 py-3 text-white bg-red-500 hover:bg-red-600 rounded-xl transition-colors font-medium shadow-md"
                      >
                        <i className='bx bx-log-out text-xl'></i>
                        Logout
                      </button>
                    </div>
                  </>
                ) : (
                  <button
                    onClick={openLoginModal}
                    className={`
                      w-full lg:w-auto px-6 py-2.5 rounded-full font-semibold text-sm transition-all duration-300
                      /* Mobile */
                      bg-main text-white hover:bg-main-dark active:scale-95 shadow-md
                      /* Desktop */
                      lg:shadow-none lg:bg-transparent lg:border-2 lg:px-6
                      ${
                        scrolled
                          ? 'lg:border-main lg:text-main lg:hover:bg-main lg:hover:text-white'
                          : 'lg:border-white lg:text-white lg:hover:bg-white lg:hover:text-main'
                      }
                    `}
                  >
                    Login
                  </button>
                )}

              </li>
            </ul>
          </nav>
        </div>
      </header>

      <AuthModal isOpen={showAuthModal} onClose={closeLoginModal} />
    </>
  );
};

export default Navbar;