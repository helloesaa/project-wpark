  ...
  return (
    <div className="min-h-screen bg-cream">
      <div className="relative w-full h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="slot-header-content relative z-10 text-center px-4 max-w-4xl mx-auto mt-16"> 
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-dark-text mb-6 leading-tight drop-shadow-sm">
      ...
      <div className="slot-grid-container px-6 md:px-24 pb-24 -mt-10 relative z-20">
        <div className="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100">
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-gray-100 pb-6">
            <h3 className="text-2xl font-bold text-gray-800 flex items-center gap-2 mb-4 md:mb-0">
              <i className='bx bxs-map-alt text-main'></i> {slotContent.grid.title}
            </h3>
            <div className="flex gap-6 text-sm font-medium bg-gray-50 px-4 py-2 rounded-full border border-gray-200">
              <span className="flex items-center gap-2 text-green-700">
                <div className="w-3 h-3 bg-green-500 rounded-full shadow-green-200 shadow-lg"></div> 
                {slotContent.grid.legend.available}
              </span>
              <span className="flex items-center gap-2 text-red-700">
                <div className="w-3 h-3 bg-red-500 rounded-full shadow-red-200 shadow-lg"></div> 
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {slots.map((slot) => (
              <div 
                key={slot.id} 
                className={`
                  relative h-40 rounded-2xl flex flex-col items-center justify-center border-2
                ...
                `}
              >
                <i className={`bx ${slot.occupied ? 'bx-car' : 'bx-check-circle'} text-5xl mb-3 transition-transform duration-300 group-hover:scale-110`}></i>
                <span className={`text-xl font-bold ${slot.occupied ? 'text-red-500' : 'text-green-700'}`}>
                  {slot.id}
                </span>
                <span className="text-xs font-semibold mt-1 uppercase tracking-wider opacity-70">
                  {slot.occupied ? slotContent.grid.legend.occupied : slotContent.grid.legend.available}
                </span>
                <div className={`absolute top-3 right-3 w-3 h-3 rounded-full ${slot.occupied ? 'bg-red-500' : 'bg-green-500 animate-pulse'}`}></div>
              </div>
            ))}
          </div>
        </div>
